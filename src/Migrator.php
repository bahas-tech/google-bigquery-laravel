...
class Migrator extends Base
{
    ...
    protected function runUp($file, $batch, $pretend)
    {
        ...
        $this->write(Info::class, "<info>Migrated:</info>  {$name}");
    }
    ...
    public function rollback($paths = [], array $options = [])
    {
        ...
        if (count($migrations) === 0) {
            $this->write(Info::class, '<info>Nothing to rollback.</info>');
    ...
    protected function rollbackMigrations(array $migrations, $paths, array $options)
    {
        ...
        foreach ($migrations as $migration) {
            if (!$file = Arr::get($files, $migration['migration'])) {
                $this->write(Info::class, "<fg=red>Migration not found:</> {$migration['migration']}");
    ...
    protected function runDown($file, $migration, $pretend)
    {
        ...
        $this->write(Info::class, "<comment>Rolling back:</comment> {$name}");
        ...
        $this->write(Info::class, "<info>Rolled back:</info>  {$name}");
    }
}
